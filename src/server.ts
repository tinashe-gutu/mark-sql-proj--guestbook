...
app.get("/signatures", async (req, res) => {
  const signatures = getSignature(); //FIXME-TASK: get signatures from db!
  res.status(200).json({
    status: "success",
    data: {
      signatures,
...
app.get("/signatures/:id", async (req, res) => {
  ...
  const signature = await getSignatureById(id); //FIXME-TASK get the signature row from the db (match on id)
...
app.post("/signatures", async (req, res) => {
  ...
  if (typeof signature === "string") {
    const createdSignature = await setSignature(signature, message); //FIXME-TASK: insert the supplied signature object into the DB
...
app.put("/signatures/:id", async (req, res) => {
  ...
  if (typeof signature === "string") {
    const result: any = await updateSignatureById(signature, message, id); //FIXME-TASK: update the signature with given id in the DB.
    console.log(result);
...
app.delete("/signatures/:id", async (req, res) => {
  ...
  const queryResult: any = await deleteSignature(id); ////FIXME-TASK: delete the row with given id from the db
